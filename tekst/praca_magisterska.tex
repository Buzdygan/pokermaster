...
\pi_i^{\sigma}(s) \cdot \pi_c(s) &= \sum\limits_{E_{(s', s)}} \pi_i^{\sigma}(s') \cdot \pi_c(s') \cdot prob_i(E_{(s', s)}) \cdot prob_c(E_{(s', s)})
\end{align*}

\noindent
Czyli żeby obliczyć wartości $\pi_i^{\sigma}(s)$ oraz $\pi_i^{\sigma}(s) \cdot \pi_c(s)$ w stanie $s$, wystarczy
mieć je policzone dla każdego stanu $s'$ z którego prowadzi krawędź do $s$. Graf stanów to tzw "DAG", czyli
...
Metoda $computeProbs$ przegląda stany w porządku topologicznym i oblicza powyższe wartości. \\

\noindent
Metoda licząca wartości $\mu^{\sigma}(s)$ i zaktualizowanie map $R$ i $S$ jest analogiczna do metody
$WalkTree$ ze standardowej wersji algorytmu przy czym każdy stan jest odwiedzany tylko raz.
...
\bibitem[3]{cfr} M.Zinkevich, M.Johanson, M.Bowling, C.Piccione $\;\;\;$ "Regret Minimization in Games with Incomplete
