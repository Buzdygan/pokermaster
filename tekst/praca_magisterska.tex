...
\begin{algorithmic}
    ...
    \Comment{$h$ to aktualna historia rozgrywki}
        ...
        \Comment{Jeśli $h$ jest końcowa}
            \State \Return $\mu(h)$
            \Comment{Zwraca wektor zysków graczy ($\mu_0$, $\mu_1$)}
        \EndIf
        ...
        \Comment{akcja gracza losowego}
            \For {$a \, \in \, A(h)$}
                ...
                \Comment{obliczamy zysk po wykonaniu akcji $a$}
                \State $\vec{\mu}(h) \gets \vec{\mu}(h) + f_c(h, a) \cdot \vec{\mu}^a$
                \Comment{aktualizujemy zysk końcowy}
            \EndFor
        \Else
            \State $o \gets O(h)$
            ...
            \For {$a \, \in \, A(h)$}
                ...
                \Comment{aktualizujemy regret akcji $a$}
                \State $S(I, a) \gets S(I, a) + \sigma_p(I, a) \cdot \pi_{p}^{\sigma}$
                \Comment{aktualizujemy uśrednioną strategię}
            \EndFor
            \For {$a \, \in \, A(h)$}
    ...
    \Comment{Zwracamy zysk końcowy dla historii $h$}
    \EndFunction
\end{algorithmic}
...
\begin{algorithmic}
    ...
    \Comment{$s$ to aktualnie odwiedzany stan}
        ...
        \Comment{Ustawiamy stan $s$ jako odwiedzony}
        ...
        \Comment{Przegląda stany sąsiadujące z $s$}
            ...
            \Comment{Obliczamy oczekiwane zyski graczy w stanie $s'$}
